...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO student_details (`Class`, `Session`, `Id`, `Name`, `Father's name`, `Mother's name`, `Dob`, `Height`, `Weight`, `Address`, `Pin`, `Phone`, `Email`) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    $stmt->bind_param("sssssssssssss", $class, $session, $collegeId, $name, $fathersName, $mothersName, $dob, $height, $weight, $address, $pin, $phone, $collegeEmail);

    // Execute query and check for success
    if ($stmt->execute()) {
        echo "<script>alert('Data submitted successfully!'); window.location.href = 'form.php';</script>";
        exit;
    } else {
        echo "<script>alert('Error: Could not submit data!');</script>";
    }
...
<head>
    ...
    <style>
        ...
        input, select {
            ...
            width: 95%; /* Adjusted width to maintain equal spacing on both sides */
            padding: 10px;
...
<body>
    <div class="container">
   <img src="bv_logo.jpg" alt="Banasthali Vidyapith Logo" class="logo"  width= "180" height= "150">
   <!-- <img src="bv_logo1.jpg" alt="Banasthali Vidyapith Logo" class="logo"  width= "150" height= "150"> -->
        ...
        <form method="POST" action="">
         ...
         <h6 center class="disclaimer">
            (This is the required form to make your profile active for this app. 
